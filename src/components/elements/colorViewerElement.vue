...
<template>
  <SelectableColor :color="color" v-model:selecting="color.selectingColorViewer"
                   :active="toolStore.tools.colorsTool.state.active" v-if="color.visible">
    <div class="color-viewer-element bi" :style="style_container"
      ...
      <div class="bi icon" :class="get_selected_icon_class()"
           :style="{color: (color.highlighted) ? 'var(--color-highlighted)' : 'black'}"></div>
    </div>
  </SelectableColor>
</template>
